...
class Box:
    num=0
    def __init__(self, boxLength, boxHeight, boxWidth):
        self.boxLength = boxLength
        self.boxHeight = boxHeight
        self.boxWidth = boxWidth
        self.start_point = gp_Pnt(0, 4000*Box.num, 0) 
        self.shape = BRepPrimAPI_MakeBox(self.start_point, 12000, 2000, 2566).Shape()
        Box.num+=1
        
class Elevation:
    def __init__(self, elevation_height, height, truss_height, upper_extension_length=0, lower_extension_length=0, degree=30, scale_factor=100):
        self.elevation_height = elevation_height
        self.height = height
        self.truss_height = truss_height
        self.upper_extension_length = upper_extension_length
        self.lower_extension_length = lower_extension_length
        self.degree = degree
...
# 分割第几个梯子
def splitElevation(elevation,elevationSplit,i):
    # 创建分割面
    all_points = calculate_split_points(elevation.elevation_height, elevation.height, elevation.upper_extension_length, elevation.degree, elevation.lower_slope_length, elevation.scale_factor)
    split_idx=get_split_idx(all_points, elevation.elevation_height, elevation.height, elevation.truss_height, elevation.upper_extension_length, elevation.lower_extension_length, elevation.degree, elevation.scale_factor)
    if len(split_idx)!=0:
        ...
        direction = gp_Dir(-1/math.tan(math.radians(elevation.degree)), 0, 1)
        ...
        splitter.AddArgument(elevation.shape[i])  # 添加要被分割的立体
...
def isInBox(shape, box):
    ...
    if(min_x>=0 and max_x<=box.boxLength and min_z>=0 and max_z<=box.boxWidth):
        return True
    return False
...
def midPack(elevationSplit,split_points,display):
    # 0是下 1 是上----------------------------------------------------------------------------------------------！！！！！！！！！！！！
...
# 如果中间切分失败  一端最长
def bestLongPack(elevationSplit,split_points,display):
    # 0是下 1 是上----------------------------------------------------------------------------------------------！！！！！！！！！！！！
    # display.DisplayShape(elevationSplit[0], update=True,color='black', transparency=0.2)
    fourPoint=get_upper_coords(split_points, 6000, 1018, 982, 2566, 2199, 30)
...
# 三段切分
def threePack(elevationSplit, display, boxes):
    # 0是下 1 是中 2是上----------------------------------------------------------------------------------------------！！！！！！！！！！！！
    # display.DisplayShape(elevationSplit[0], update=True,color='black', transparency=0.2)
    rotated_down=rotate_shape(elevationSplit[0],330)
    ...
    print("是否在箱子内：",isInBox(inBoxUp, boxes[0]))
    print("是否在箱子内：",isInBox(inBoxdown, boxes[0]))
    print("是否在箱子内：",isInBox(anotherBoxmid, boxes[1]))
